...
2.  **TLS Certificates**: Domain-specific self-signed certificates are generated via Terraform using the `hashicorp/tls` provider.
3.  **Application Deployment**: Resources are deployed via a robust shell script triggered by `local-exec`.
    *   **Odoo**: Deployed as a `Deployment`.
    *   **PostgreSQL**: Deployed as a `StatefulSet` with a headless Service for stable networking.
    *   **Ingress**: Exposes the application over HTTPS using the generated TLS certificates.
...
-   **StatefulSet for DB**: PostgreSQL is deployed as a StatefulSet to ensure data persistence and stable network identity, complying with best practices for stateful applications.
-   **Security**: All endpoints are secured with generic self-signed TLS certificates generated by Terraform. HTTP-only access is disabled via Ingress annotations.
...
### 4. Validate
Verify that all endpoints are reachable and serving the application correctly over HTTPS.
```bash
make validate
```
...
1.  Open `main.tf`.
2.  Locate the `client_config` variable.
3.  Add a new key-value pair for the client and their requested environments.
    ```hcl
    variable "client_config" {
      default = {
        # ... existing clients ...
        tesla = ["dev", "staging", "prod"]
      }
    }
    ```
4.  Initialize the new workspace:
    ```bash
    terraform workspace new tesla
    ```
5.  Apply the configuration:
    ```bash
    make apply client=tesla
    ```

### How to Add a New Environment
1.  Open `main.tf`.
2.  Locate the `client_config` variable.
3.  Add the new environment string to the client's list.
    ```hcl
    variable "client_config" {
      default = {
        airbnb = ["dev", "prod", "staging"] # Added 'staging'
      }
    }
    ```
4.  Apply the changes (Terraform will create the new cluster and leave existing ones):
    ```bash
    make apply client=airbnb
    ```

### Validation Details
Validation is performed using `curl`. Since we use self-signed certificates, validation checks must allow insecure connections (`-k`).
Manual check example:
```bash
curl -k -v https://odoo.dev.nike.local
```
